...
use agv_vehicle_simulator::{config, log, map, socket};
...
/// One `VehicleSimulator` + TCP session to the scheduler (same spawn pattern as `vda5050_vehicle_simulator`).
async fn spawn_vehicle_simulator(
    ...
    tokio::spawn(async move {
        socket::tcp_client::connection_loop(cfg, agv_id, map, lt).await;
    });
}
