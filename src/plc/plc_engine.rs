...
impl PlcProtobufEngine {
    pub fn new(config: Config, agv_id: i32, map: Option<Arc<MapModel>>, log_target: String) -> Self {
        let mut eng = Self {
            config: config.clone(),
            agv_id,
            map: map.clone(),
    ...
    fn apply_initial_pose(&mut self) {
        ...
        if let (Some(m), Some(name)) = (
            ...
            if let Some((xm, ym)) = m.point_world(name) {
                self.x_m = xm;
                self.y_m = ym;
                self.last_open_tcs_point = name.to_string();
            ...
            warn!(
                target: self.lt(),
                "map.initial_point_name {:?} not found; using protobuf initial x/y mm",
                name
            );
        }
    ...
    pub(crate) fn try_start_route(&mut self, segment_ids: &[i32]) -> bool {
        ...
        if self.last_open_tcs_point.is_empty() {
            warn!(
                target: self.lt(),
                "Cannot build plant route: last_open_tcs_point empty; set map.initial_point_name"
            );
            return false;
        }
